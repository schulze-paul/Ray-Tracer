...
Color view_distance_shader(const Ray &r, HittableList &world, std::shared_ptr<HittableList>& lights, Background &background, int depth_)
{
    double max_depth = infinity;
    HitRecord rec;
    if (!world.hit(r, 0.001f, max_depth, rec))
    {
        return Color(1, 1, 1);
    }
    
    double depth = rec.getT();
