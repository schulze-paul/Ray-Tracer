...
std::vector<Color> ray_tracing_shader(const Ray &r, HittableList &world, std::shared_ptr<HittableList>& lights, Background &background, int depth)
{
    HitRecord rec;
    // max depth reached
    if (depth <= 0)
        return std::vector<Color> {Color(0, 0, 0)};
    
    // background
    if (!world.hit(r, 0.001f, 100.0f, rec))
        return std::vector<Color> {background.get_color(r)};
    ...
    if (!rec.getMaterial()->scatter(r, rec, albedo, scattered, pdf_value))
        return std::vector<Color> {emitted};

    std::vector<Color> colors;
    if (!rec.getMaterial()->isLambertian())
        colors = ray_tracing_shader(scattered, world, lights, background, depth - 1);
        for (int i = 0; i < colors.size(); i++)
            colors[i] = emitted + albedo * colors[i];
        return colors;
    ...
    {
        scattered = Ray(rec.getHitPoint(), directions[i], r.get_time());
        auto ray_colors = ray_tracing_shader(scattered, world, lights, background, depth - 1);
        for (int i = 0; i < ray_colors.size(); i++)
            colors.push_back(emitted + albedo * ray_colors[i] / pdf_values[i]);
    }
    return colors;
}
