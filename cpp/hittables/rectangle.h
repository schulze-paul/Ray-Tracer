...
bool XY_Rectangle::hit(const Ray &r, double t_min, double t_max, HitRecord &rec) const
{
    ...
    if (dot(r.direction, normal) > 0){ normal = -normal;}
...
Triangle::Triangle(const Vec3 &p0, const Vec3 &p1, const Vec3 &p2, Material *material)
{
    ...
    this->p1 = p1;
    this->p2 = p2;
    this->normal = unit_vector(cross(p1 - p0, p2 - p0));
    this->m = material;
}
...
public:
    ...
    {
        // compute face normals with undetermined orientation
        ...
        // body center is known to be inside the prism
        ...
        Vec3 face_center_3 = (p0 + p3 + p1 + p4) / 4.0;
        Vec3 face_center_4 = (p0 + p3 + p2 + p5) / 4.0;
        Vec3 face_center_5 = (p1 + p4 + p2 + p5) / 4.0;

        // We know that the vector from the body center to the face center
        // points outside. We can use this to compare with the respective
        // normal vector, which should also point outside
        if (dot(face_center_1 - body_center, face_normal_1) < 0)
            face_normal_1 = -face_normal_1;
        if (dot(face_center_2 - body_center, face_normal_2) < 0)
            face_normal_2 = -face_normal_2;
        if (dot(face_center_3 - body_center, face_normal_3) < 0)
            face_normal_3 = -face_normal_3;
        if (dot(face_center_4 - body_center, face_normal_4) < 0)
            face_normal_4 = -face_normal_4;
        if (dot(face_center_5 - body_center, face_normal_5) < 0)
...
TriangularPrism::TriangularPrism(const Vec3 &p0, const Vec3 &p1, const Vec3 &p2, const Vec3 &p3, const Vec3 &p4, const Vec3 &p5, Material *material)
{
    ...
    sides.add(std::make_shared<Triangle>(p0, p4, p3, m));
    sides.add(std::make_shared<Triangle>(p1, p2, p5, m));
    sides.add(std::make_shared<Triangle>(p1, p5, p4, m));
...
{
    return sides.hit(r, t_min, t_max, rec);
    // set the normal direction to pointing outside
    Vec3 point_normal_offset = 0.01 * rec.get_normal() + rec.get_hit_point();
