...
/**
 * @brief      Class for rotate x.
 */
class RotateX : public Hittable
{
public:
    RotateX(std::shared_ptr<Hittable> hittable, double angle)
    {            
        this->hittable = hittable;
        auto radians = degrees_to_radians(angle);
        sin_theta = std::sin(radians);
...
bool RotateX::hit(const Ray &r, double t_min, double t_max, HitRecord &rec) const
{
    ...
    p[1] = cos_theta * rec.get_hit_point()[1] + sin_theta * rec.get_hit_point()[2];
    p[2] = -sin_theta * rec.get_hit_point()[1] + cos_theta * rec.get_hit_point()[2];

    normal[1] = cos_theta * rec.get_normal()[1] + sin_theta * rec.get_normal()[2];
    normal[2] = -sin_theta * rec.get_normal()[1] + cos_theta * rec.get_normal()[2];
...
/**
 * @brief      Class for rotate z.
 */
class RotateZ : public Hittable
{
public:
    RotateZ(std::shared_ptr<Hittable> hittable, double angle)
    {            
        this->hittable = hittable;
        auto radians = degrees_to_radians(angle);
        sin_theta = std::sin(radians);
...
bool RotateZ::hit(const Ray &r, double t_min, double t_max, HitRecord &rec) const
{
    ...
    p[0] = cos_theta * rec.get_hit_point()[0] - sin_theta * rec.get_hit_point()[1];
    p[1] = sin_theta * rec.get_hit_point()[0] + cos_theta * rec.get_hit_point()[1];

    normal[0] = cos_theta * rec.get_normal()[0] - sin_theta * rec.get_normal()[1];
    normal[1] = sin_theta * rec.get_normal()[0] + cos_theta * rec.get_normal()[1];
